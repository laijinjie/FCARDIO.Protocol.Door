...
namespace DotNetDrive.Protocol.POS.Test
{
    public partial class frmTime : frmNodeForm
    {
        ...
        private void BtnReadSystemTime_Click(object sender, EventArgs e)
        {
            ...
            {
                var result = cmde.Command.getResult() as ReadTime_Result;
        ...
        private void BtnWriteSystemTime_Click(object sender, EventArgs e)
        {
            var cmdDtl = mMainForm.GetCommandDetail();
            if (cmdDtl == null) return;
            WriteCustomTime_Parameter par = new WriteCustomTime_Parameter();
            par.ControllerDate = DateTime.Parse(txtComputerTime.Text);
            WriteTime cmd = new WriteTime(cmdDtl, par);
            mMainForm.AddCommand(cmd);
        }
        #endregion
    }
