...
namespace FCARDIO.Protocol.Elevator.FC8864.SystemParameter.FunctionParameter
{
    ...
    public class WriteReadCardSpeak_Parameter : AbstractParameter
    {
        ...
        public override bool checkedParameter()
        {
            ...
            if (SpeakSetting.MsgIndex != 1 && SpeakSetting.MsgIndex != 2)
            {
                return false;
            }
            if (SpeakSetting.BeginDate > SpeakSetting.EndDate)
            {
                return false;
            }
            if (SpeakSetting.BeginDate.Year > 2099 || SpeakSetting.BeginDate.Year < 2000)
            {
                return false;
            }
            if (SpeakSetting.EndDate.Year > 2099 || SpeakSetting.EndDate.Year < 2000)
            {
                return false;
            }
            return true;
